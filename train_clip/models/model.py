...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=device) if self.attn_mask is not None else None
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        return self.visual(image.type(self.dtype).to(device))

    def encode_text(self, text):
        x = self.token_embedding(text).type(self.dtype).to(device)  # [batch_size, n_ctx, d_model]
